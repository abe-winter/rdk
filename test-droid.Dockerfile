...
RUN --mount=type=cache,target=/var/cache/apt apt-get install -qy unzip
# sdkmanager requires 21 jre, we build on old jdk because we target old droid. (not sure latter is necessary)
RUN --mount=type=cache,target=/var/cache/apt apt-get install -qy openjdk-11-jdk-headless openjdk-21-jre-headless
